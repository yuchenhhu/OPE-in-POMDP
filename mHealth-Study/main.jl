...
function genmod_on(n, capT, u = reward, burn = 50, init_states = 999)
    ...
    for i = 1:n 
        ...
        eps = rand(d_er, 1)[1]
        S[i, 1, 2] = (1 - beta[1]) * 100 + beta[1] * (S[i, 1, 1]-eps_id) + beta[2] * S[i, 2, 1] + beta[3] * S[i, 2, 1] + beta[4] * S[i, 3, 1] + beta[5] * S[i, 3, 1] + beta[6] * A[i, 1] + beta[7] * A[i, 1] + eps + eps_id
        if S[i, 1, 2] > 250
        ...
        for t=3:capT
            ...
            eps = rand(d_er, 1)[1]
            S[i, 1, t] = (1 - beta[1]) * 100 + beta[1] * (S[i, 1, t - 1]-eps_id) + beta[2] * S[i, 2, t - 1] + beta[3] * S[i, 2, t - 2] + beta[4] * S[i, 3, t - 1] + beta[5] * S[i, 3, t - 2] + beta[6] * A[i, t - 1] + beta[7] * A[i, t - 2] + eps + eps_id
            if S[i, 1, t] > 250
...
function off_est(chain_on,n,t,k)
    ...
    if k==-1
        ...
        #est = ifelse.(weights_roll.==0, 0., sum(chain_r_weighted,dims=2)./sum(weights_roll,dims=2))
        return est
    end
end
